...
activity_type = {
	...
	# Should the player get an Alert for being able to join this activity if Open (can be toggled off in UI)
	# Default is no
	notify_player_can_join_open_activity = no

	# Foldable activity group type where to put this activity in the gui.
	# Defaults to the 'activities' group.
	activity_group_type = <key>

	# The order that the activity should show up within its activity group in the gui.
	...
	# Possible filters:
	#	* capital: the planner's capital
	#	* domain: provinces in the planner's domain
	#	* realm: provinces in the planner's realm
	#	* top_realm: provinces in the planner's top liege's realm
	#	* holy_sites: holy sites for the planner's faith
	#	* holy_sites_domain: holy sites for the planner's faith, or their domain
	#	* holy_sites_realm: holy sites for the planner's faith, or their realm
	#	* domicile: location of the domicile
	#	* domicile_domain: location of the domicile AND provinces in the domain
	#	* domicile_realm: location of the domicile AND provinces in the realm
	#	* top_liege_border_inner: provinces of the top liege's realm directly bordering other realms
	#	* top_liege_border_outer: provinces directly bordering the top liege's realm
	#	* landed_title: de jure provinces under a specific landed title (needs to specify a target, landless titles such as noble families are ineligible)
	#	* geographical_region: a specific geographical_region (needs to specify a target)
	#	* all: all provinces in the world
	#
	...
	# Defaults to same as the base filter. If a province_filter_target must be specified for province_filter, you probably
	# want ai_province_filter to be the same.
	# WARNING: You really definitely do not want the AI using all...
	ai_province_filter = filter

	# If you specify a province_filter or ai_province_filter that needs a specific target, use this field to specify the target.
	# the same target will be used for both province_filter and ai_province_filter, which need to be the same in these cases.
	# 	* if you set landed_title: use the key of a landed title ( e.g. k_england )
	# 	* if you set geographical_region: use the key of a region ( e.g. nile_river_region )
	...
	options = {
		...
		option_category = {
			...
			option_1 = {
				...
				# List of activity phases that are removed from the activity if this option is picked
				# Optional and only used for activities that have no pickable phases.
				blocked_phases = {
					archery joust
				}
	...
	phases = {
		archery = {
			# If this phase is predefined: always present and not removable unless by picking options that affect phases (see above)
	...
	# How often will the AI check if it can host this particular activity in months? (for all tiers)
	ai_check_interval = 1
	### Brief: ai_check_interval_by_tier (int31 per tier)
	# How often should the ai check  if it can host this particular activity.
	# '0' means it is never considered. Used instead of 'ai_check_interval'. All tiers are required to be defined.
	...
	# available variables:
	# 	scope:<option_category_key> = for every option category defined, will have a flag of the selected option
	# warning: these flags may not be set, so usage of ?= is advised.
	max_guests = {
		value = 10

		if = {
			limit = {
				scope:option_category ?= flag:option_1
			}
			add = 20
		}
	}

	# How many of the guest slots will be reserved for characters added via effects or events, so that the guest list will not be completely filled up
	# when we add them
	...
	# Note: The AI will only start joining by themselves once all invites from `guest_invite_rules` are handled.
		...
		key = { # Will be localized as activity_window_character_key
			...
			scripted_animation = {
				...
				triggered_animation = {
					...
					### brief: camera ( database key, optional )
					# A camera type defined for a triggered animation will override the default
					# camera defined for the portrait if that animation is chosen
					#
					camera = camera_name
				}
